using System;
using System.Collections.Generic;
using System.Linq;
using System.Runtime.Serialization;
using System.ServiceModel;
using System.Text;

namespace WcfMapReduceService
{
    // NOTE: You can use the "Rename" command on the "Refactor" menu to change the interface name "IMapReduceService" in both code and config file together.
    [ServiceContract( SessionMode=SessionMode.Required, CallbackContract =typeof(IMapReduceCallback) )]    
    public interface IMapReduceService
    {
        [OperationContract]
        List<KeyValuePair<string,int>> MapReduce(List<string> data, List<string> outputKeys);
        
    }
}
